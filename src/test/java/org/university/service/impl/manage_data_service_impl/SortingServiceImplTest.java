...
class SortingServiceImplTest {
    ...
    private static SortingServiceImpl service;
    ...
    static void setUp() {
        ...
        service = new SortingServiceImpl();
    ...
    @Test
    void sortCompaniesByCollectedFeesDesc_ordersBySumThenName() {
        Company cA = persistCompany("A");
        Company cB = persistCompany("B");

        Employee eA = persistEmployee(cA, "E", "A");
        Employee eB = persistEmployee(cB, "E", "B");

        Building bA = persistBuilding(eA, "BA");
        Building bB = persistBuilding(eB, "BB");

        Apartment a1 = persistApartment(bA, "Room: 1001");
        Apartment a2 = persistApartment(bB, "Room: 2001");

        Invoice i1 = persistInvoice(a1, YearMonth.of(2025, 1), new BigDecimal("200.00"), PaymentStatus.NOT_PAID);
        Invoice i2 = persistInvoice(a2, YearMonth.of(2025, 1), new BigDecimal("50.00"), PaymentStatus.NOT_PAID);

        payInvoice(i1.getId());
        payInvoice(i2.getId());

        List<CompanyRevenueDto> res = service.sortCompaniesByCollectedFeesDesc();
        assertNotNull(res);
        assertTrue(res.size() >= 2);
        assertEquals(cA.getId(), res.get(0).getCompanyId());
        assertEquals(0, new BigDecimal("200.00").compareTo(res.get(0).getCollectedFees()));
    }

    @Test
    void sortEmployeesByCompanyByName_orders() {
        Company c = persistCompany("C1");
        Employee e2 = persistEmployee(c, "Ivan", "Z");
        Employee e1 = persistEmployee(c, "Georgi", "A");

        List<Employee> res = service.sortEmployeesByCompanyByName(c.getId());
        assertNotNull(res);
        assertEquals(2, res.size());
        assertEquals(e1.getId(), res.get(0).getId());
        assertEquals(e2.getId(), res.get(1).getId());
    }

    @Test
    void sortEmployeesByCompanyByBuildingsCountDesc_ordersByCount() {
        Company c = persistCompany("C1");
        ...
        List<EmployeeBuildingsCountDto> res = service.sortEmployeesByCompanyByBuildingsCountDesc(c.getId());
        assertNotNull(res);
        assertEquals(2, res.size());
    ...
    @Test
    void sortResidentsByBuildingByName_and_byAge_orders() {
        Company c = persistCompany("C1");
        Employee e = persistEmployee(c, "E", "E");
        Building b = persistBuilding(e, "Building");
        Apartment a = persistApartment(b, "Room: 1001");

        persistResident(a, "Ivan", 40);
        persistResident(a, "Georgi", 20);
        persistResident(a, "Ana", 30);

        List<Resident> byName = service.sortResidentsByBuildingByName(b.getId());
        assertEquals(3, byName.size());
        assertEquals("Ana", byName.get(0).getFirstName());

        List<Resident> byAge = service.sortResidentsByBuildingByAge(b.getId());
        assertEquals(3, byAge.size());
        assertEquals(20, byAge.get(0).getAge());
    }
}
